...
public:
  ...
  {
    const int GLOBAL_ENABLE_TRACE_LEVEL = 42;

    if (m_delay_map.contains(delay_name)) {
      uint32_t delay_usec = m_delay_map[delay_name];
      if (delay_usec > 0) {
        // check if the user has set the appropriate TRACE level, which we use as a global enable
        if (TTEST(GLOBAL_ENABLE_TRACE_LEVEL)) {
