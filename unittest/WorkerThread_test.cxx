...
BOOST_AUTO_TEST_CASE(sanity_checks)
{
  ...
  std::unique_ptr<dunedaq::utilities::WorkerThread> umth_ptr = nullptr;

  auto starttime = std::chrono::steady_clock::now();
  BOOST_REQUIRE_NO_THROW(umth_ptr = std::make_unique<dunedaq::utilities::WorkerThread>(do_something));
...
BOOST_AUTO_TEST_CASE(inappropriate_transitions, *boost::unit_test::depends_on("sanity_checks"))
{
  ...
  BOOST_REQUIRE_THROW(umth.stop_working_thread(), dunedaq::utilities::ThreadingIssue);

  umth.start_working_thread();

  BOOST_REQUIRE_THROW(umth.start_working_thread(), dunedaq::utilities::ThreadingIssue);
...
{
  dunedaq::utilities::WorkerThread umth(print_name);
