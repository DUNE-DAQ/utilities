...
BOOST_AUTO_TEST_CASE(sanity_checks)
{
  ...
  std::unique_ptr<dunedaq::toolbox::ThreadHelper> umth_ptr = nullptr;

  auto starttime = std::chrono::steady_clock::now();
  BOOST_REQUIRE_NO_THROW(umth_ptr = std::make_unique<dunedaq::toolbox::ThreadHelper>(do_something));
...
BOOST_AUTO_TEST_CASE(inappropriate_transitions, *boost::unit_test::depends_on("sanity_checks"))
{
  ...
  dunedaq::toolbox::ThreadHelper umth(do_something);
  BOOST_REQUIRE_THROW(umth.stop_working_thread(), dunedaq::toolbox::ThreadingIssue);

  umth.start_working_thread();

  BOOST_REQUIRE_THROW(umth.start_working_thread(), dunedaq::toolbox::ThreadingIssue);
...
{
  dunedaq::toolbox::ThreadHelper umth(print_name);
